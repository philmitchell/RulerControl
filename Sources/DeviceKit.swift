...
public enum Device {
  ...
  /// Points-per-inch on the current device's screen (if applicable). When the device is not applicable this property returns nil.
  public var pointsPerInch: CGFloat? {
      if let ppi = self.ppi {
          return CGFloat(ppi) / UIScreen.main.scale
      }
      return nil
  }
